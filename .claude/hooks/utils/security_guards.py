#!/usr/bin/env python3
"""
Security guards for teach-niche-v0 dangerous command and file access detection.

Provides pattern matching to detect potentially destructive operations:
- Dangerous rm commands (rm -rf, recursive deletion)
- Access to sensitive files (.env, credentials, Stripe API keys, Supabase passwords)

Used by PreToolUse hooks to warn or block risky operations.

TODO: Customize sensitive patterns for teach-niche stack:
- Stripe API keys (STRIPE_SECRET_KEY, STRIPE_WEBHOOK_SECRET)
- Supabase credentials (SUPABASE_SERVICE_ROLE_KEY, DATABASE_URL)
- Next.js env files (.env.local, .env.production)
"""
...
# TODO: Add teach-niche-specific patterns below
SENSITIVE_FILE_PATTERNS = [
    r"\.env",
    r"\.env\.",
    r"\.env\.local",       # Next.js local environment
    r"\.env\.production",  # Next.js production environment
    r"credentials",
    r"secret",
    r"\.aws/credentials",
    r"\.ssh/id_",
    # TODO: Uncomment and customize for teach-niche
    # r"STRIPE_SECRET_KEY",
...
def is_env_file_access(tool_name: str, tool_input: dict) -> bool:
    ...
    if content and tool_name == "Write":
        ...
        # TODO: Add teach-niche-specific patterns
        for pattern in ["password", "api_key", "token", "secret", "STRIPE_", "SUPABASE_"]:
