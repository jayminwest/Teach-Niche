...
describe("EditLesson Component", () => {
  ...
  test("submits form successfully", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/Lesson updated successfully!/i))
        .toBeInTheDocument();
    }, { timeout: 3000 });
  ...
  test("handles delete confirmation", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole("button", { name: /delete lesson/i }))
        .toBeInTheDocument();
    });
    ...
    // Wait for success message
    await waitFor(() => {
      expect(screen.getByText(/Lesson deleted successfully!/i))
        .toBeInTheDocument();
    }, { timeout: 3000 });
