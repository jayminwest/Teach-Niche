...
describe('CancelPage Component', () => {
  ...
  test('renders main sections and is accessible', () => {
    ...
    // Test main heading
    expect(screen.getByRole('heading', { name: /Purchase Canceled/i })).toBeInTheDocument();
  ...
  test('displays correct cancel message', () => {
    ...
    expect(screen.getByText(/You can try purchasing again from the marketplace/i)).toBeInTheDocument();
  });

  test('return to marketplace button navigates correctly', () => {
    renderWithProviders(<CancelPage />);
    
    const marketplaceButton = screen.getByRole('button', { name: /Return to Marketplace/i });
  ...
  test('button is properly styled', () => {
    ...
    const button = screen.getByRole('button', { name: /Return to Marketplace/i });
