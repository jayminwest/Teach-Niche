...
global.fetch = jest.fn(() =>
  Promise.resolve({
    ok: true,
    json: () =>
      Promise.resolve({ sessionUrl: "https://checkout.stripe.com/test" }),
  })
);
...
describe("LessonCard Component", () => {
  ...
  test("renders lesson card with correct information", () => {
    ...
    expect(screen.getByText(`Creator: ${defaultProps.creatorName}`))
      .toBeInTheDocument();
    expect(screen.getByText(`$${defaultProps.price.toFixed(2)}`))
      .toBeInTheDocument();
  ...
  test("handles image load error", () => {
    renderWithRouter(
      <LessonCard {...defaultProps} thumbnail_url="invalid-url" />,
    );
