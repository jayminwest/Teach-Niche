  ...
  try {
    if (req.method === "POST") {
      ...
      if (!body.user_id || body.user_id !== user.id) {
        console.error("Invalid user_id", { provided: body.user_id, expected: user.id });
      ...
      const authUrl = `https://api.vimeo.com/oauth/authorize?response_type=code&client_id=${
    ...
    if (req.method === "GET") {
      ...
      const tokenResponse = await fetch("https://api.vimeo.com/oauth/access_token", {
        ...
        headers: {
          ...
          "Authorization": `Basic ${btoa(`${Deno.env.get("VIMEO_CLIENT_ID")}:${Deno.env.get("VIMEO_CLIENT_SECRET")}`)}`
        },
        body: JSON.stringify({
